    ...
    **kwargs,
    ...
    while True:
        ...
        for g, moves, score in beams:
            ...
            # cost-aware 时拉宽召回再按 (cnt - α*cost) 重排；alpha=0 保持旧召回量
            if alpha > 0:
    ...
    greedy_variants = [(False, 0.0), (True, 0.0)]
    if alpha > 0:
        greedy_variants.append((True, alpha))
    for g, moves, score in all_states:
        # 组合：纯贪心 / 前瞻无cost / 前瞻+cost
        for la, a in greedy_variants:
...
def solve_multi_alpha(
    grid: np.ndarray,
    time_budget: float = 120.0,
    target_score: int = 0,
    beam_width: int = 30,
    warm_start: tuple[list, int] | None = None,
    alphas: tuple[float, ...] | list[float] = (0.0, 0.5),
    **kwargs,
) -> tuple[list[tuple[int, int, int, int]], int]:
    """用多组 digit-cost 权重在同一总预算内尝试，返回最高分方案。

    alpha=0.0 保留旧的无 digit-cost 偏好，用来兜底；alpha>0 使用
    cost-aware 搜索。每一轮都带上当前最优解作为 warm_start，避免后续
    策略返回更差结果。time_budget 是总预算，不是每个 alpha 的预算。
    """
    if not alphas:
        alphas = (0.5,)
    ...
    for i, alpha in enumerate(unique_alphas):
        ...
        remaining_runs = len(unique_alphas) - i
        alpha_budget = remaining_budget / remaining_runs
        moves, score = solve(
            grid,
            time_budget=alpha_budget,
        ...
        log.info(
            f"alpha={alpha:g}: {score}分, {len(moves)}步, "
            f"预算{alpha_budget:.1f}s"
        )
        if score > best_score:
            best_moves, best_score = moves, score
        current_warm = (best_moves, best_score)
        if target_score > 0 and best_score >= target_score:
            break

    log.info(f"multi-alpha 最优: {best_score}分, {len(best_moves)}步")
    return best_moves, best_score
